	...
	return (
		<section className="py-6">
			<div className="flex flex-row gap-3 lg:gap-6">
				<div className="w-[250px] sm:w-[260px] lg:w-2/5 flex-shrink-0 relative overflow-hidden rounded-xl bg-gradient-to-br from-[#ffcd82] to-[#ffb142] py-3 px-3 lg:py-4 lg:pr-0 lg:pl-4 shadow-sm flex flex-col justify-between group min-h-[140px] lg:min-h-[220px]">
					...
					<div className="z-10 relative">
						<div className="inline-flex items-center gap-1 bg-[#e65100] text-white text-[10px] lg:text-xs font-bold px-2 py-1 lg:px-3 lg:py-1.5 rounded-md mb-2 lg:mb-4 uppercase tracking-wide font-body">
							<Tag size={10} />
							<span className="hidden sm:inline">Desconto</span>
							<span className="sm:hidden">-</span>
						</div>

						<h3 className="text-sm sm:text-base lg:text-2xl font-extrabold text-[#1a1a2e] leading-tight font-display">
						...
						<Link to="/servicos/aluguel-de-automoveis">
							<button className="group/btn mt-2 lg:mt-3 flex items-center gap-1 text-[#1a1a2e] font-bold border-b-2 border-[#1a1a2e] pb-0.5 w-fit hover:opacity-75 transition-opacity cursor-pointer font-body">
				...
				<div className="flex-1 min-w-0 overflow-hidden">
					<div ref={carouselRef} className="flex gap-3 lg:gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x h-full">
						{categories.map((cat) => (
							<div
								key={cat.id}
								className="min-w-[110px] sm:min-w-[150px] lg:min-w-[200px] flex-1 bg-white border border-[#e5e7eb] rounded-2xl p-3 lg:p-4 flex flex-col items-center justify-between hover:shadow-md transition-shadow cursor-pointer snap-start"
							>
								<span className="font-display font-bold text-[#1a1a2e] mt-1 text-sm lg:text-base lg:mt-2">{cat.name}</span>
			...
			<div className="flex gap-2 mt-6">
				<button onClick={() => handleScroll('left')} className="w-10 h-10 border border-[#e5e7eb] rounded-lg flex items-center justify-center text-[#154c9a] hover:bg-[#f8f6f2] hover:border-[#154c9a] transition-colors cursor-pointer">
					<ChevronLeft size={20} />
				</button>
				<button onClick={() => handleScroll('right')} className="w-10 h-10 border border-[#e5e7eb] rounded-lg flex items-center justify-center text-[#154c9a] hover:bg-[#f8f6f2] hover:border-[#154c9a] transition-colors cursor-pointer">
