...
import { useWishlist, useRemoveVehicleFromWishlist, useRemovePecaFromWishlist } from '../../hooks/queries/useWishlist';

const Favoritos = () => {
	useDocumentTitle('Favoritos - CaxiAuto');

	const { data: wishlist, isLoading } = useWishlist();
	const removeVehicle = useRemoveVehicleFromWishlist();
	const removePeca = useRemovePecaFromWishlist();
	const vehicles = wishlist?.vehicles || [];
	const pecas = wishlist?.pecas || [];
	...
	const handleRemoveVehicle = async (vehicleId) => {
		...
		try {
			const res = await removeVehicle.mutateAsync(vehicleId);
			if (res && res.success) {
	...
	const handleRemovePeca = async (pecaId) => {
		...
		try {
			const res = await removePeca.mutateAsync(pecaId);
			if (res && res.success) {
