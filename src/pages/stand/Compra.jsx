...
export default function Compra() {
	...
	const queryParams = useMemo(() => {
		const params = { page: currentPage, limit: vehiclesPerPage, sort: sortBy, type: 'SALE' };
		...
		if (filters.quilometros) {
			const [min, max] = filters.quilometros.split('-');
			if (min) params.minKilometers = parseInt(min);
			if (max) params.maxKilometers = parseInt(max);
		}
		if (filters.preco) {
	...
	const { data: response, isLoading, error: queryError } = useQuery({
		queryKey: ['vehicles', 'sale', currentPage, sortBy, filters],
	...
	const getPrice = (vehicle) => {
		if (!vehicle || !vehicle.priceSale) return null;
				...
				<div className="mx-auto max-w-7xl px-6 lg:px-8 py-24 w-full relative z-10">
					<div className="max-w-4xl mx-auto text-center">
						<div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/15 text-white mb-10">
							...
							<span className="text-sm font-semibold tracking-wide font-body">Compra de Veículos</span>
						</div>
										...
										<div className="grid grid-cols-2 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
											{vehicles.map((car) => {
												...
												return (
													...
													>
														<div className="relative h-40 overflow-hidden cursor-pointer" onClick={() => navigate(`/stand/compra/${car.id}`)}>
															<img
