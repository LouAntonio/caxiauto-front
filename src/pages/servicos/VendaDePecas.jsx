	...
	const steps = [
		{
			number: '01',
			label: 'PASSO',
			title: 'Pesquisa Precisa',
			description: 'Encontre exatamente o que procura filtrando por marca, modelo, ano ou referência da peça.',
			icon: Search,
		},
		{
			number: '02',
			...
			description: 'Visualize ofertas de múltiplos fornecedores para a mesma peça e escolha o melhor preço ou condição.',
			icon: GitCompare,
		},
		{
			number: '03',
			label: 'PASSO',
			title: 'Garanta a Compatibilidade',
			description: 'Verifique detalhes técnicos e compatibilidade com a sua viatura antes de fechar negócio.',
			icon: ClipboardCheck,
		},
		{
			number: '04',
			label: 'PASSO',
			title: 'Compra Direta',
			description: 'Contacte o fornecedor diretamente através da plataforma e combine a entrega ou recolha.',
			icon: ShoppingBag,
		}
	];
	...
	const features = [
		{
			...
			description: 'Catálogo de peças genuínas de fabricantes reconhecidos para garantir a máxima qualidade e durabilidade.'
		},
		{
			icon: Coins,
			title: 'Opções Económicas',
			description: 'Alternativas de marcas brancas ou recondicionadas que oferecem excelente relação qualidade-preço.'
		},
		{
			icon: Layers,
			title: 'Múltiplas Marcas',
			description: 'Stock abrangente para viaturas europeias, asiáticas e americanas de todos os segmentos.'
		},
		{
			icon: UserCheck,
			title: 'Vendedores Confiáveis',
			description: 'Rede de fornecedores e sucatas devidamente verificados para sua total segurança.'
		},
	];

	const categories = [
		{ name: 'Motor & Transmissão', desc: 'Componentes mecânicos essenciais', icon: Settings },
		{ name: 'Travagem & Suspensão', desc: 'Segurança e estabilidade', icon: Disc },
		{ name: 'Elétricos & Luzes', desc: 'Baterias, sensores e faróis', icon: Zap },
		{ name: 'Interiores & Conforto', desc: 'Estofos, consolas e acessórios', icon: Car },
		{ name: 'Manutenção', desc: 'Óleos, filtros e fluidos', icon: Droplets },
		{ name: 'Carroçaria', desc: 'Para-choques, portas e vidros', icon: Layers }, // Using Layers as generic body parts
	];
		...
		<main className="bg-gradient-to-b from-white to-gray-50 min-h-screen">
			...
			<section className="py-24 px-6 bg-[#154c9a] text-white relative overflow-hidden isolate">
				...
				<div className="max-w-4xl mx-auto text-center relative z-10">
					<div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-100 mb-6 backdrop-blur-sm">
						...
						<span className="font-medium">Marketplace de Peças</span>
					</div>
					<h1 className="text-5xl md:text-7xl font-extrabold mb-8 tracking-tight">
						Peças & Acessórios
					</h1>
					<p className="text-xl md:text-2xl text-blue-100 max-w-3xl mx-auto leading-relaxed">
						Tudo o que o seu carro precisa num só lugar. Peças originais, alternativas e usadas de fornecedores de confiança.
					</p>

					<div className="mt-10 flex flex-wrap justify-center gap-4">
						{/* Placeholder search bar could go here */}
						<div className="relative max-w-md w-full">
							...
							<input
								type="text"
								disabled
								className="block w-full pl-11 pr-4 py-4 border-transparent bg-white/10 backdrop-blur-md rounded-full text-white placeholder-blue-200 focus:outline-none focus:ring-2 focus:ring-white/50 focus:bg-white/20 transition-all border border-white/20"
								placeholder="Em breve: Pesquise por referência ou peça..."
							/>
						</div>
					</div>
				</div>
			...
			<section className="py-24 px-6 overflow-hidden">
				<div className="max-w-5xl mx-auto relative">
					<div className="text-center mb-20">
						...
						<h2 className="text-4xl md:text-5xl font-bold text-gray-900">Como comprar na Caxiauto</h2>
					</div>
			...
			<section className="py-24 px-6 bg-white relative">
				...
				<div className="max-w-7xl mx-auto">
					<div className="text-center mb-20">
						<span className="text-[#154c9a] font-bold tracking-wider uppercase text-sm mb-2 block">Diferenciais</span>
						<h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">Encontre o que precisa</h2>
						<p className="text-xl text-gray-600 max-w-2xl mx-auto">
							Soluções completas para a manutenção ou reparação do seu veículo.
						</p>
					</div>
			...
			<section className="py-24 px-6 bg-slate-900 text-white relative overflow-hidden">
				...
				<div className="max-w-7xl mx-auto relative z-10">
					<div className="text-center mb-16">
						<h2 className="text-3xl md:text-4xl font-bold mb-4">Catálogo Abrangente</h2>
						<p className="text-blue-100/80">Temos componentes disponíveis para todas as áreas da sua viatura</p>
					</div>

					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
						{categories.map((cat, idx) => (
							<div key={idx} className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl border border-white/10 hover:bg-white/20 transition-all flex items-center gap-4 group">
								<div className="p-3 rounded-xl bg-[#154c9a] shadow-lg group-hover:scale-110 transition-transform duration-300">
									<cat.icon className="w-6 h-6 text-white" />
								</div>
								<div>
									<h3 className="text-lg font-bold">{cat.name}</h3>
									<p className="text-sm text-blue-100 mt-1">{cat.desc}</p>
								</div>
							</div>
						))}
					</div>
